...
- A paused or new head waiting past `urgentWaitMs` (off by default) is urgent: it outranks every non-urgent paused or new program, urgent heads go oldest first, and under `origin-only` it may take any pod with room. It still needs a pod with room. This is the port's deadline tier (proposal Part 1 aging plus Part 3 Option B); `thunder_agent_urgent_promotions_total` counts its dispatches.
...
| `urgentWaitMs` | `0` (off) | none | Urgent tier: a paused or new head that waited this long is ordered ahead of all non-urgent paused and new heads (oldest first) and, under `origin-only`, may take any pod with room. Fit-checked. Must be below `headWaitStarvationMs`. |
...
- **Urgent tier.** `urgentWaitMs` reorders waiting programs by age once they have waited that long and frees them from origin-only placement; upstream orders the resume pool by size only and has no deadline notion.
