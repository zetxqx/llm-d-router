...
| `originWaitMaxMs` | `0` (never) | none | With `origin-only`: a paused program that has waited this long for its origin pod may take the pod with the most room. Ordering unchanged. Set it inside the time a paused prefix survives on its pod (about 8 s at 32 sessions per pod on the measured pool). |
